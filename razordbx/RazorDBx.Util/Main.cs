...
namespace RazorUtil
{
	public class Header
	{
		...
		{
			Console.WriteLine ("=== RazorDBx Utility v0.0.2 ===\n");
			...
			if (args.Length == 0) {
				...
				case "dump-journal":
					if (args.Length < 3) Console.WriteLine ("Invalid parameters"); else DumpJournal (args [1], int.Parse (args [2]));
					break;
				case "dump-table":
					if (args.Length < 4) Console.WriteLine ("Invalid parameters"); else DumpFile (args [1], int.Parse (args [2]), int.Parse (args [3]));
					break;
				case "dump-manifest":
					if (args.Length < 2) Console.WriteLine ("Invalid parameters"); else {
				...
				case "dump-manifest-all":
					if (args.Length < 2) Console.WriteLine ("Invalid parameters"); else {
						...
						foreach (var mf in Manifest.ReadAllManifests(args[1])) mf.LogContents (dummyMf);
					}
					break;
				case "split-manifest":
					if (args.Length < 2) Console.WriteLine ("Invalid parameters"); else {
						...
						foreach (var mf in Manifest.ReadAllManifests(args[1])) {
							using (var bw = new BinaryWriter(new FileStream(Path.Combine(args[1], "S" + ct.ToString() + ".mf"), FileMode.CreateNew, FileAccess.Write, FileShare.None, 40096))) mf.WriteManifestContents (bw);
							ct++;
						}
					}
					break;
				case "check-each-table":
					if (args.Length < 2) Console.WriteLine ("Invalid parameters"); else CheckBlockTableFiles (args [1]);
					break;
				case "check-database":
					if (args.Length < 2) Console.WriteLine ("Invalid parameters"); else CheckDatabase (args [1]);
					break;
				case "remove-orphans":
					if (args.Length < 2) Console.WriteLine ("Invalid parameters"); else RemoveOrphanedTables (args [1]);
					break;
				case "remove-page":
					if (args.Length < 4) Console.WriteLine ("Invalid parameters"); else {
		...
		static void DumpJournal (string baseDir, int version)
		{
			...
			foreach (var pair in journal.Enumerate()) Console.WriteLine ("{0} => {1}", pair.Key.ToString (), pair.Value.ToString ());
		}
	}
}