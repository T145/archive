...
namespace RazorDBxTests
{
	...
	public class CacheTests
	{
		...
		public void CheckOverSizeLimit ()
		{
			...
			for (int i = 0; i < 100; i++) Assert.IsFalse (cache.TryGetValue (items [i], out output));

			// Next 100 items should still be there
			for (int i = 100; i < 200; i++) Assert.IsTrue (cache.TryGetValue (items [i], out output));
		}
		...
		public void CheckLRU ()
		{
			...
			for (int i = 0; i < 50; i++) Assert.IsTrue (cache.TryGetValue (items [i], out output));
			...
			for (int i = 50; i < 60; i++) Assert.IsFalse (cache.TryGetValue (items [i], out output));

			// Next 50 items should still be there
			for (int i = 60; i < 110; i++) Assert.IsTrue (cache.TryGetValue (items [i], out output));
