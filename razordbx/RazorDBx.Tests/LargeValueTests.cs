...
namespace RazorDBxTests
{
	...
	public class LargeValueTests
	{
		...
		byte[] GenerateBlock (int num)
		{
			...
			for (int i = 0; i < num; i++) block [i] = (byte)(i & 0xFF);
		...
		void CheckBlock (byte[] bytes)
		{
			...
			for (int i = 0; i < 20; i++) Assert.AreEqual (checksum [i], bytes [i + num], "Checksum[{0}] = {1} differs from bytes[{2}] = {3}", i, checksum [i], i + num, bytes [i + num]);
		}
		...
		public void TestLargeAndSmallEvenWrites ()
		{
			...
			using (var db = new KeyValueStore(path)) {
				...
				for (int i = 0; i < keys.Count; i++) {
					...
					var v = db.Get (k);
					CheckBlock (v);
					if ((i & 1) == 1) Assert.Less (v.Length, 100, " i = {0} should be small, but size={1}", i, v.Length); else Assert.Greater (v.Length, 100, " i = {0} should be large, but size={1}", i, v.Length);
				}
			}
		}
		...
		public void TestLargeAndSmallInterlacedWrites ()
		{
			...
			// Create a random set of keybytes
			List<byte[]> keys = new List<byte[]> ();
			for (int i = 0; i < 10; i++) keys.Add (new Key (new byte[] { (byte)i, (byte)i }, 0).KeyBytes);
			...
			using (var db = new KeyValueStore(path)) {
				...
				for (int i = 0; i < keys.Count; i++) {
					var k = keys [i];
					if (((i & 1) == 0)) db.Set (k, GenerateBlock (Config.MaxLargeValueSize - 100)); else db.Set (k, GenerateBlock (10));
				}

				// Set Odds to large
				for (int i = 0; i < keys.Count; i++) {
					var k = keys [i];
					if (((i & 1) == 1)) db.Set (k, GenerateBlock (Config.MaxLargeValueSize - 100)); else db.Set (k, GenerateBlock (10));
				}
