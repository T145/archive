...
namespace RazorDBx
{
	public static class MergeEnumerator // This class takes a collection of IEnumerable<T> objects (assuming in sorted order) and returns a merged version with the result being in sorted order as well.
	{
		public static IEnumerable<T> Merge<T> (IEnumerable<IEnumerable<T>> enumerables) // Merge the specified enumerables.
		{
			return Merge (enumerables, o => o);
		}

		public static IEnumerable<T> Merge<T, TKey> (IEnumerable<IEnumerable<T>> enumerables, Func<T, TKey> keyExtractor) // Merge the specified enumerables and keyExtractor.
		{
			...
			foreach (var e in enumerators)
				if (e.Value.MoveNext ())
					...
					e.Value.Dispose (); // Move ahead and remove empty enumerators.

			Comparison<Ranked<IEnumerator<T>>> comparer = (x, y) => { // Construct the expression to compare the enumerators, taking rank into account.
				int c = Comparer<TKey>.Default.Compare (keyExtractor (x.Value.Current), keyExtractor (y.Value.Current));
				if (c == 0)
					return x.Rank.CompareTo (y.Rank);
				else
					return c; // If any enumerators are equal, then compare their ranks.
			};

			// Order the enumerators by the first (current) element and put into a linked list.
			...
			try {
				...
				while (totalEnumerators > 0) {
					...
					if (!firstEnum.Value.Value.MoveNext ()) { // Advance this enumerator to the next spot.
						firstEnum.Value.Value.Dispose (); // Remove the completed enumerator.
