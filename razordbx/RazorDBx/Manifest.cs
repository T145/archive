...
namespace RazorDBx
{
	public class ManifestImmutable : IDisposable
	{
		...
		public Key FindSpanningLimit (int level, Key startingKey) // Finds the maximum key that we can span to avoid covering to much upper level data.
		{
			if (level >= NumLevels)
				return Key.Empty; // Invalid level check.
			if (_pages [level].Count == 0)
				return Key.Empty; // Indicates no page limits.

			var index = FindPageIndexForKey (level, startingKey); // Find the page for the starting key.
			if (index < 0) {
				// The startingKey is before the first page, so treat it as if the first page contains the key (even though no pages contain the startingKey): The startingKey is after the last page, so don't limit the span otherwise.
			...
			if (_pages [level].Count > maxIndex)
				return _pages [level] [maxIndex].LastKey; // The last key in that page is the furthest we can span at the lower level.

			return Key.Empty; // There aren't enough pages to trigger the limit.
		}
		...
		public ManifestImmutable NextVersion (int level, out int version) // Mutation Methods - Must return a new copy of the manifest.
		{
			if (level >= MaxLevels)
				throw new IndexOutOfRangeException ();
			var m = Clone ();
		...
		public ManifestImmutable ModifyPages (IEnumerable<PageRecord> addPages, IEnumerable<PageRef> removePages) // Atomically add/remove page specifications to/from the manifest.
		{
			var m = Clone ();
	...
	public class Manifest
	{
		...
		ManifestImmutable LastManifest {
			get {
				return _manifests.Last.Value;
			}
		}
		...
		public int NextVersion (int level) // atomically acquires the next version and persists the metadata
		{
			lock (manifestLock) {
				int version;
		...
		public PageRecord NextMergePage (int level) // atomically acquires the next merge page and persists the metadata
		{
			lock (manifestLock) {
				PageRecord page;
		...
		public void ModifyPages (IEnumerable<PageRecord> addPages, IEnumerable<PageRef> removePages) // Atomically add/remove page specifications to/from the manifest
		{
			lock (manifestLock) {
	...
	public struct PageRef : IComparable<PageRef>
	{
		...
		public int CompareTo (PageRef other) // Define the order of pages by the merging priority (higher priority is "less than").
		{
			int r = Level.CompareTo (other.Level); // Compare first on level, lower level is higher priority.
			if (r == 0)
				return -Version.CompareTo (other.Version);
			else
				return r; // Compare next on version, higher version is higher priority.
		}
	}
	...
	public class PageRecord
	{
		...
		public int RefCount {
			get {
				return _snapshotReferenceCount;
			}
		}
